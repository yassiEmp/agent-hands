...
export function explain(v) {
  return [
    ...
    '',
    '  Staying attached here is the one moment it reliably costs you: sign-in is',
    '  where a site grades hardest, and that flag is the cheapest thing it reads.',
    '  Disconnected without touching the page.',
    '',
    '  Pick one:',
    '    agent-hands launch <url>        relaunch clean; the flag disappears at',
    '                                    the source and CDP is then safe here',
    '    agent-hands login --window ...  sign in through the OS layer, unattached',
    '    --force                         stay attached anyway, knowing the cost',
  ].join('\n');
}
